...
/** Production-grade utilities for reading data from tables and data sources.
  *
  * This object provides type-safe, scalable methods for reading data in both local and production
  * environments. It automatically handles environment detection and applies appropriate reading
...
object Fetchers {
  ...
  /** Reads a table as a DataFrame with comprehensive error handling.
    *
    * In local mode: Reads from in-memory table storage In production mode: Reads from the
    * configured catalog (e.g., Hive, Iceberg)
    *
    * @param tableName
    *   Name of the table to read (must be non-empty)
    * @return
    *   DataFrame containing the table data
    * @throws IllegalArgumentException
    *   if tableName is empty or invalid
    * @throws RuntimeException
    *   if table is not found or read operation fails
    */
  ...
  /** Reads a table as a strongly-typed Dataset with compile-time type safety.
    *
    * In local mode: Reads from in-memory table storage and converts to specified type In production
    * mode: Reads from the configured catalog and converts to specified type
    *
    * @param tableName
    *   Name of the table to read (must be non-empty)
    * @param encoder
    *   Implicit encoder for type T (provided automatically by Spark)
    * @tparam T
    *   The case class type to deserialize the data into
    * @return
    *   Dataset[T] containing the strongly-typed table data
    * @throws IllegalArgumentException
    *   if tableName is empty or invalid
    * @throws RuntimeException
    *   if table is not found, read operation fails, or type conversion fails
    */
  ...
  /** Reads a table with optional column selection for optimized performance.
    *
    * This method allows you to specify which columns to read, reducing I/O and memory usage for
    * large tables where only a subset of columns is needed.
    *
    * @param tableName
    ...
    * @throws IllegalArgumentException
    *   if tableName is empty or columns list is empty
    * @throws RuntimeException
    *   if table is not found or read operation fails
    */
