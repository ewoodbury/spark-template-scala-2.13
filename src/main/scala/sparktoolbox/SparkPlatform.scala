...
/** Production-grade Spark platform for managing SparkSession lifecycle and table operations.
  *
  * This object provides a centralized way to manage Spark sessions with support for both local
  * development and production environments. It automatically detects the environment and applies
  * appropriate configurations for optimal performance at scale.
  *
  * Key features:
...
object SparkPlatform extends SparkPlatformTrait {
  ...
  /** Determines if the platform is running in local mode. Local mode is enabled when SPARK_MODE
    ...
    *
    * @return
    *   true if in local mode, false for production mode
    */
  ...
  /** Gets or creates the SparkSession with appropriate configurations for the environment.
    ...
    *
    * @return
    *   Thread-safe SparkSession instance
    * @throws RuntimeException
    *   if SparkSession creation fails
    */
  ...
  /** Creates a new SparkSession with environment-appropriate configurations.
    *
    * @return
    *   Try[SparkSession] containing either the created session or the failure
    */
  ...
  /** Creates a SparkSession optimized for production environments. Includes Iceberg configuration
    * for production data lake operations.
    */
  ...
  /** Retrieves a locally stored table/dataset by name. Used primarily in local mode for in-memory
    * table storage during development and testing.
    *
    * @param tableName
    *   Name of the table to retrieve
    * @return
    *   Some(Dataset) if the table exists, None otherwise
    */
  ...
  /** Stores a dataset locally by name. Used primarily in local mode for in-memory table storage
    * during development and testing.
    *
    * @param tableName
    *   Name to store the table under
    * @param dataset
    *   Dataset to store
  ...
  /** Clears all locally stored tables. Used for cleanup between test runs or when resetting the
    * local environment.
    */
  ...
  /** Stops the Spark session and cleans up all resources.
    *
    * This method should be called when shutting down the application to ensure proper cleanup of
    * resources and prevent resource leaks.
    *
  ...
  def stop(): Unit =
    synchronized {
      _spark.foreach { session =>
        Try(session.stop()).recover { case ex =>
  ...
  /** Gets basic information about the current Spark session. Useful for debugging and monitoring.
    *
    * @return
