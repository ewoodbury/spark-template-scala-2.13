...
/** Production-grade platform provider that automatically selects the appropriate SparkPlatform
...
object PlatformProvider {
  ...
  /** Gets the appropriate SparkPlatform implementation for the current environment.
    *
    * The selection logic:
    *   1. If running in a test context (ScalaTest or sbt testing), attempts to load
    *      TestSparkPlatform 2. If TestSparkPlatform is not available on classpath, falls back to
    *      production platform 3. If not in test context, uses production platform
    *
    * This design ensures that production deployments never have test dependencies, while
    * development and testing environments get enhanced capabilities.
    *
    * @return
    *   SparkPlatformTrait implementation appropriate for the current environment
    */
  ...
  /** Attempts to load the test platform implementation using reflection.
    *
    * This approach allows the production code to remain completely clean of test dependencies while
    * still providing enhanced test capabilities when the test classpath is available.
    *
    * @return
    *   Some(TestSparkPlatform) if successfully loaded, None otherwise
    */
  ...
  /** Determines if the current execution context is a test environment.
    ...
    *   1. Thread name contains "ScalaTest" (ScalaTest execution) 2. System property "sbt.testing"
    *      is set to "true" (sbt test execution)
    *
    * @return
  ...
  /** Gets information about the current platform selection for debugging purposes.
    *
    * @return
    *   Map containing platform selection details
    */
