 ...
 * 
 * This object provides type-safe, scalable methods for writing data in both local
 * and production environments. It automatically handles environment detection and
 * applies appropriate writing strategies for optimal performance at terabyte scale.
 * 
 * Key features:
...
object WriteUtils {
   ...
   * 
   * In local mode: Stores in in-memory table storage
   * In production mode: Writes to the configured catalog with optimizations
   * 
   * @param df DataFrame to write (must be non-null)
   * @param tableName Name of the target table (must be non-empty)
   * @param mode SaveMode for the write operation (default: Overwrite)
   ...
   * In production mode: Writes to the configured catalog with optimizations
   * 
   * @param ds Dataset to write (must be non-null)
   * @param tableName Name of the target table (must be non-empty)
   * @param mode SaveMode for the write operation (default: Overwrite)
   ...
   * 
   * Partitioning is crucial for performance at terabyte scale as it enables
   * partition pruning during reads, dramatically reducing scan times.
   * 
   * @param df DataFrame to write
   * @param tableName Name of the target table
   * @param partitionColumns Sequence of column names to partition by
   * @param mode SaveMode for the write operation (default: Overwrite)
   * @throws IllegalArgumentException if inputs are invalid
   * @throws RuntimeException if write operation fails
   */
  def writeDataFrameToTablePartitioned(
    ...
    require(missingColumns.isEmpty, s"Partition columns not found in DataFrame: ${missingColumns.mkString(", ")}")
    ...
    Try {
      ...
      case Failure(exception) => 
        throw new RuntimeException(
          s"Failed to write partitioned DataFrame to table '$tableName' with partitions [${partitionColumns.mkString(", ")}]: ${exception.getMessage}", 
          exception
        )
    }
  }
   ...
   * @param df DataFrame to write
   * @param tableName Name of the target table
   * @param mode SaveMode for the write operation
   * @param targetFileSizeMB Target file size in MB (default: 128MB)
   */
   ...
   */
  private def writeToLocalStorage(data: org.apache.spark.sql.Dataset[_], tableName: String, mode: SaveMode): Unit = {
    mode match {
